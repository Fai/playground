...
describe('Challenge - The Secret Source', function () {
    ...
    it('Execution', async function () {
        ...
        const secretSlot = 1; // the slot starts from 0, ValixPlaygroundToken is 0, secretSource is 1
        const secret = await ethers.provider.getStorageAt(theSecretSource.address, secretSlot);
        await theSecretSource.connect(player).guess(secret);
    });
